...
const CarouselSection = ({ title, username }) => {
  async function buy(index) {
    ...
    try {
      ...
      console.log("Connected to Polygon:", accounts);
      ...
      const gas = await contract.methods.buySkin(index, username).estimateGas({ from: accounts[0], value: price });

      // Call the buySkin function in the contract with the appropriate parameters
      const transaction = await contract.methods.buySkin(index, username).send({ from: accounts[0], value: price, gas: gas });
      
      console.log("Transaction successful. Transaction Hash:", transaction.transactionHash);
    } catch (error) {
