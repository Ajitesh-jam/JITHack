...
const CarouselSection = ({ title, username }) => {
  async function buy(index) {
    ...
    try {
      ...
      const gas = await contract.methods.buySkin(index, username).estimateGas({ from: account, value: price });

      // Call the buySkin function in the contract with the appropriate parameters
      const transaction = await contract.methods.buySkin(index, username).send({ from: account, value: price, gas: gas });
